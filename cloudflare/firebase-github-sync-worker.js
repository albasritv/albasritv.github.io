...
export default {
  async fetch(request, env) {
    ...
    if(request.method==="GET" && url.pathname==="/health"){
      return json({ok:true,service:"bsr-instant-sync",mode:"direct-github"},200);
    }
    ...
    try{
      ...
      const cats=values(categories).filter(c=>c&&c.scope==="LIVE")
      ...
      const sections=cats.map(c=>{
        const list=allChannels.filter(ch=>String(ch.categoryId||"")===String(c.id||""))
          .sort((a,b)=>(a.order??999999)-(b.order??999999)||String(a.title||"").localeCompare(String(b.title||"")))
          .map(ch=>({
      ...
      const matches=values(publicMatches).filter(Boolean).filter(m=>m.important!==false).map(m=>{
        ...
        return {
          id:m.id||"",matchId:m.matchId||"",matchKey:m.matchKey||"",homeName:m.homeName||"",awayName:m.awayName||"",
          homeLogo:m.homeLogo||"",awayLogo:m.awayLogo||"",baghdadTime:m.baghdadTime||m.meccaTime||"",
          meccaTime:m.meccaTime||m.baghdadTime||"",sourceDate:m.sourceDate||"",status:m.status||"",score:m.score||"",
          tournament:m.tournament||"",important:m.important!==false,channels:unique
        };
      ...
      const catalog={version:Date.now(),updatedAt:now,source:"firebase-direct-worker",sections};
      await commitFiles(env.GITHUB_TOKEN,{
        "app/data/catalog-live.json":JSON.stringify(catalog,null,2),
        "app/data/matches-live.json":JSON.stringify(matches,null,2)
      });

      return json({ok:true,instant:true,sections:sections.length,channels:sections.reduce((n,s)=>n+s.channels.length,0),matches:matches.length,updatedAt:now},200);
    }catch(e){
...
async function fb(path){
  const r=await fetch(FIREBASE+path+".json?ts="+Date.now(),{headers:{"Cache-Control":"no-cache"}});
...
async function commitFiles(token,files){
  ...
  const ref=await fetch(refUrl,{headers}).then(checkJson);
  ...
  const newCommit=await fetch(`https://api.github.com/repos/${OWNER}/${REPO}/git/commits`,{
